...
function Member4() {
  ...
  const loadSongsFromLocalStorage = () => {
    const storedSongs = localStorage.getItem('songs4');
    return storedSongs ? JSON.parse(storedSongs) : [
      { id: 1, songTitle: 'Fantasy', artistName: 'Khai Dreams' },
      { id: 2, songTitle: 'Six Feet Under', artistName: 'Billie Eilish' },
      { id: 3, songTitle: 'Symphony(feat. Zara Larsson)', artistName: 'Clean Bandit' },
      { id: 4, songTitle: 'josee!', artistName: 'Damons Year' },
      { id: 5, songTitle: '별이 되지 않아도 돼', artistName: '109' },
    ];
  };
  ...
  const saveSongsToLocalStorage = (songs) => {
    localStorage.setItem('songs4', JSON.stringify(songs));
  };
  ...
  const onChangeMood = function (event) {
    ...
    localStorage.setItem("moodStateM4", selectedMood);
  }

  useEffect( () => {
    const savedMood = localStorage.getItem("moodStateM4");
    if (savedMood) {
      setMood(savedMood);
    }
  }, []);
